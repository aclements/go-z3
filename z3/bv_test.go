...
func TestBVAsBig(t *testing.T) {
	...
	val := ctx.FromBigInt(big.NewInt(255), ctx.BVSort(8))
	big1 := val.BVUAsBig()
	if big1.String() != "255" {
	...
	val = ctx.FromBigInt(big.NewInt(-1), ctx.BVSort(8))
	big1 = val.BVUAsBig()
	if big1.String() != "255" {
		t.Errorf("expected 255, got %s", big1)
	}
...
func TestBVAsInt(t *testing.T) {
	...
	x = ctx.FromInt(255, ctx.BVSort(9))
	vu, isConst, ok = x.BVAsUint()
	if vu != 255 || !isConst || !ok {
	...
	x = ctx.FromInt(255, ctx.BVSort(64))
	vu, isConst, ok = x.BVAsUint()
	if vu != 255 || !isConst || !ok {
	...
	x = ctx.FromInt(-1, ctx.BVSort(128))
	vu, isConst, ok = x.BVAsUint()
	if vu != 0 || !isConst || ok {
